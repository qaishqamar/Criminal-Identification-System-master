...
def basicPageSetup(pageNo):
    ...
    back_img = tk.PhotoImage(file="E:\\Criminal-Identification-System-master\\Criminal-Identification-System-master\\back.png")
...
def selectMultiImage(opt_menu, menu_var):
    global img_list, current_slide, slide_caption, slide_control_panel, RegisterChoice
    RegisterChoice="Select Images"
    ...
    if(len(path_list) < 5):
        ...
        back_img = tk.PhotoImage(file="E:\\Criminal-Identification-System-master\\Criminal-Identification-System-master\\previous.png")
        next_img = tk.PhotoImage(file="E:\\Criminal-Identification-System-master\\Criminal-Identification-System-master\\next.png")
...
def getPage1():
    ...
    btn_grid.pack()
# 
    tk.Button(btn_grid, text="Select Images",  font="Arial 15 bold", bg="#2196f3",
           fg="white", pady=10, bd=0, highlightthickness=0,command=lambda: selectMultiImage(opt_menu, menu_var), activebackground="#091428",
           activeforeground="white").grid(row=0, column=0, padx=25, pady=25)

    tk.Button(btn_grid, text="Capture Faces",  font="Arial 15 bold", bg="#2196f3",
...
# 
    tk.Button(scroll_frame, text="Register",  font="Arial 15 bold",command=lambda: register(entries, required, menu_var),
           bg="#2196f3", fg="white", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#091428",
           activeforeground="white").pack(pady=25)
def getEntries(entries,required):
...
def register(entries, required, menu_var): 
    global img_list
    entry_data=getEntries(entries,required)  
    # Setting Directory
    
    if RegisterChoice=="Select Images":
        ...
        rowId=insertData(entry_data)
        profile_igmg=generate_dataset(img_list,entry_data)
        if(rowId > 0):
            messagebox.showinfo("Success", "Criminal Registered Successfully.")
            # shutil.move(path, os.path.join('face_samples', entry_data["Name"]))
            ...
            if not os.path.isdir("profile_pics"):
                os.mkdir("profile_pics")
            cv2.imwrite("profile_pics/criminal %d.png"%rowId,profile_igmg )

            
        
        goBack()
def generate_dataset(img_list,entry_data):
        ...
        img_id=0
        # crete directory
        if not isdir('face_samples/%s'%entry_data['Name']):
            os.makedirs('face_samples/%s'%entry_data['Name'])
        ...
        while True:
            ...
            if face_cropped(frame) is not None:
                ...
                facePic=face.copy()
                # if img_id<=5:
                #     img_list[img_id]=face
                face  = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
               
                file_name_path = "face_samples/%s/"%entry_data["Name"]+str(img_id)+".png"
...
logo = tk.PhotoImage(file = "C:\\Users\\qamar\\Downloads\\Criminal-Identification-System-master\\Criminal-Identification-System-master\\logo.png")
